...
epoch = 25
main_path = "C:/Users/gnrno/PycharmProjects/pythonBase/DB3/"
...
for lr in learning_rate:
    for fn in fold_name:
        ...
        inception = tf.keras.applications.VGG16(
            include_top=False,
            weights="imagenet",
        ...
        early_stopping = EarlyStopping(monitor="accuracy", patience=5, min_delta=0.0001, mode="max")
        ...
        model.save(
            main_path + 'VGG16Results/_VGG16_' + str(epoch) + '_' + str(ooptimizer) + '_' + str(lr) + '_' + fn + '')
        ...
        sns.heatmap(conf, annot=True, cmap='Blues', annot_kws={'fontsize': 13}, xticklabels=confusion_labels,
                    yticklabels=confusion_labels, )
        plt.ylabel('Gercek Değer')
        plt.xlabel('Tahmini Değer')
        plt.title(title)
        plt.savefig(
            main_path + 'VGG16Results/_confusion_matrix_VGG16_' + str(epoch) + '_' + str(ooptimizer) + '_' + str(
                lr) + '_' + fn + '_' + str(time) + 'dk_' + str(accuracy) + '%.png', format="png", dpi=300,
            bbox_inches='tight')
